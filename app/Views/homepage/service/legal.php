<!-- Start of Banner -->
<div class="banner-service">
    <div class="wrapper-service">
        <video autoplay="" preload="" muted="" loop="" playsinline="">
            <source src="<?= BASE_URL ?>assets/vid/banner-legal.webm" type="video/webm">
        </video>
        <div class="banner-text-video">
            <div class="bg-text-video legal">
                <h2 class="f-lato">
                    Legal And Tax Advice 
                    <br>
                </h2>
            </div>

        </div>
    </div>
</div>
<!-- End of Banner -->

<!-- Start of Short Desc -->
<section class="content legal-short-desc">
    <div class="container">
        <div class="row row-cols-1">
            <div class="col-12 col-md-10 mx-auto">
                <h1 id="g-legalshortdesc-title" class="f-odor">
                    <span class="line-animation">
                        Legal, Tax and Specialized Accounting Consulting        
                    </span>
                </h1>
                <h3 id="g-legalshortdesc-subtitle" class="f-odor">
                    <span class="line-animation">
                        Legal and accounting solutions for your company
                    </span>
                </h3>
                <p id="g-legalshortdesc-text" class="mt-4 f-inter"> 
                    <span class="line-animation">
                        Our lawyers, accountants and tax advisors provide specialized advice to ensure legal compliance and optimize the accounting management of your company, both locally and internationally. We understand that the legal and regulatory landscape is constantly evolving and our goal is to keep your company up to date and protected,
                    </span> 
                    <span class="line-animation">
                        Thanks to our global network of international partners in the relevant jurisdiction, we can assist you wherever you are or want to go.
                    </span>
                </p>
            </div>
        </div>
    </div>
</section>
<!-- End of Short Desc -->

<!-- Start of Service -->
<section class="service my-0">
    <!-- Service 1 -->
    <section id="g-legal-advice" class="wrapper-box-service">
        <div class="box-service brown-legal right">
            <div class="container">
                <div class="row row-cols-1 row-cols-lg-2">
                    <div class="col wrapping-title">
                        <h2 class="box-content-title f-odor">
                            Legal Advice
                            <hr>
                        </h2>
                    </div>
                    <div class="col box-content">
                        <p class="box-content-text f-inter">
                            <span>Contracts:</span> Drafting, reviewing and negotiating commercial contracts to protect your interests in every transaction.
                        </p>
                        <p class="box-content-text f-inter ">
                            <span>Corporate Law:</span> Advice on the establishment of new companies, mergers, acquisitions, corporate restructuring and other corporate transactions.
                        </p>
                        <p class="box-content-text f-inter ">
                            <span>Regulatory Compliance:</span> Assistance in adapting to local and international regulations, including privacy management and data protection.
                        </p>
                    </div>
                </div>
            </div>
        </div>
        <div class="wrapper-img-absolute right">
            <img class="img-legal-advice "src="<?= BASE_URL ?>assets/img/img-17.webp" alt="Legal Advice">
            <img class="img-legal-advice" src="<?= BASE_URL ?>assets/img/img-17.webp" alt="Legal Advice">
        </div>
    </section>


    <!-- Service 2 -->
    <section id="g-tax-advice" class="wrapper-box-service">
        <div class="box-service gold-legal left">
            <div class="container">
                <div class="row row-cols-1 row-cols-lg-2">
                    <div class="col order-1 order-lg-0 box-content">
                        <p class="box-content-text f-inter">
                            <span>Tax Planning:</span> Long-term tax strategy to optimize your position and reduce the overall tax burden.
                        </p>
                        <p class="box-content-text f-inter ">
                            <span>Advice on IVA/ VAT and Indirect Taxes:</span>Assistance in the management of indirect taxes and IVA/ VAT
                        </p>
                    </div>
                    <div class="col order-0 order-lg-1 wrapping-title">
                        <h2 class="box-content-title f-odor">
                            Tax Advice
                            <hr>
                        </h2>
                    </div>
                </div>
            </div>
        </div>
        <div class="wrapper-img-absolute left">
            <img class="img-tax-advice "src="<?= BASE_URL ?>assets/img/img-18.webp" alt="Tax Advice">
            <img class="img-tax-advice" src="<?= BASE_URL ?>assets/img/img-18.webp" alt="Tax Advice">
        </div>
    </section>
</section>
<!-- End of Service -->

<!-- Start of Short Desc -->
<section class="legal-hand">
    <div class="container">
        <div class="row row-cols-1">
            <div class="col-12 col-lg-8 mx-auto wrapper-hand">
                <h1 class="f-odor ">International Consulting Specialists</h1>
                <h2 class="f-odor">Navigating the Global Markets with Expertise</h2>
                <p class="f-inter">  
                     At <span translate='no'> P.N. Global</span>, we stand out for our ability to assist companies in their global expansion and in managing complex international challenges. Our international consulting is a strength that helps you navigate the global legal and tax landscape with confidence and expertise. We have a thorough understanding of the laws and regulations in each country where our clients wish to expand their business. Whether it is the establishment of foreign subsidiaries, management of legal and tax risks, or regulatory compliance, Our team of experts is always ready to provide personalized and strategic support. Trust us at <span translate='no'> P.N. Global </span> to make your international ambitions a reality, by taking full advantage of the opportunities offered by global markets.
                </p>
            </div>
        </div>
    </div>
</section>
<!-- End of Short Desc -->

<section class="legal-video">
    <div id="g-legal-vid-left" class="row">
        <div class="col-4"></div>
        <div class="col-8">
            <div class="wrapper-left">
                <video autoplay="" preload="" muted="" loop="" playsinline="">
                    <source src="<?= BASE_URL ?>assets/vid/legal-left.webm" type="video/webm">
                </video>
            </div>
        </div>
    </div>
    <div id="g-legal-vid-right" class="row">
        <div class="col-6 d-flex justify-content-start">
            <div class="wrapper-right">
                <video autoplay="" preload="" muted="" loop="" playsinline="">
                    <source src="<?= BASE_URL ?>assets/vid/legal-right.webm" type="video/webm">
                </video>
            </div>
        </div>
        <div class="col-6"></div>
    </div>
</section>
