<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        window.confirmPayment = function() {
            ...
            fetch('/elite/deposit/save_payment_to_session', {
                    method: 'POST',
                    body: formData
                })
                .then(response => {
                ...
                .then(data => {
                    if (data.status === 'success') {
                        ...
                        window.location.href = '/elite/deposit/payment_option';
                    } else {
        ...
        function fetchInvestmentConfig() {
            fetch('/elite/deposit/get_investment_config')
                .then(response => {
                    if (!response.ok) {
                        throw new Error('Network response was not ok');
                    }
        ...
        fetchInvestmentConfig();
            ...
            var tableTotalMember = $('#table_deposithistory').DataTable({
            // processing: true,
            serverSide: false,
            responsive: true,
            paging: true,
            ...
            ajax: {
                url: '<?= BASE_URL ?>elite/deposit/get_history',
                type: 'GET',
