...
<script>
    ...
    // Cek apakah ada OTP di session
    document.addEventListener("DOMContentLoaded", function() {
        const phoneNumber = "<?= $phone_number ?>";

        // tampilkan modal kalau nomor kosong
        if (!phoneNumber || phoneNumber.trim() === "") {
            openWhatsappModal();
        }

        const selectCountryCode = document.getElementById('country_code');
        const inputPhoneNumber = document.getElementById('phone_number');
        const hiddenFullPhone = document.getElementById('full_phone');
        ...
        // urutkan negara
        countryData.sort((a, b) => a.code.localeCompare(b.code));

        // isi dropdown
        countryData.forEach(country => {
            const option = document.createElement('option');
            option.value = country.dial_code;
            option.textContent = `(${country.dial_code}) ${country.code}`;
            selectCountryCode.appendChild(option);
        });

        // default Indonesia
        const indonesiaOption = countryData.find(country => country.code === 'ID');
        if (indonesiaOption) {
            selectCountryCode.value = indonesiaOption.dial_code;
        }

        // update hidden input
        function updateFullPhoneNumber() {
            let countryCode = selectCountryCode.value.replace('+', ''); // hapus +
            let phoneNumber = inputPhoneNumber.value.replace(/\D/g, ''); // hanya angka
            hiddenFullPhone.value = countryCode + phoneNumber;
        }

        selectCountryCode.addEventListener('change', updateFullPhoneNumber);
        inputPhoneNumber.addEventListener('input', updateFullPhoneNumber);

        updateFullPhoneNumber();

        // simpan nomor
        document.getElementById("saveWhatsappBtn").addEventListener("click", async function() {
            updateFullPhoneNumber(); // pastikan hidden sudah terisi
            const fullPhone = hiddenFullPhone.value;

            const isConfirm = confirm("OTP will be sent to WhatsApp number: " + fullPhone);

            if (!isConfirm) {
                return; // user klik Cancel -> stop
            }

            try {
                const res = await fetch("<?= BASE_URL ?>hedgefund/dashboard/add_whatsapp_number", {
                    method: "POST",
                    headers: {
                ...
                if (res.ok && data.success) {
                    alert("Whatsapp number saved successfully, please check your WhatsApp for the OTP.");
                    closeWhatsappModal();
                    location.reload();
                } else {
